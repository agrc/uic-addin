...
public static class LayerService {
    ...
    public static Layer GetLayer(string name, Map map) {
        ...
        var match = map.GetLayersAsFlattenedList().FirstOrDefault(layer => {
            if (layer is not BasicFeatureLayer) {
                return false;
            }
    ...
    private static async Task<SelectionSet> GetSelection(Map map) {
    ...
    private static async void RemoveSelections(Map map) {
        ...
        foreach (var pair in selection.ToDictionary()) {
    ...
    public static async Task SetSelectionFromId(long id, Layer layer) => await
        ThreadService.RunOnBackground(() => layer.Map.SetSelection(SelectionSet.FromDictionary(new Dictionary<MapMember, List<long>> {
            {
                layer, new List<long> {
                    id
                }
